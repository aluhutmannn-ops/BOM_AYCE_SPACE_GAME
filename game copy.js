...
const enemyWidth=canvas.width0.06, aspect=choice.img.height/choice.img.width, enemyHeight=enemyWidthaspect;
const base={type:choice.type,x:canvas.width+n60,y:Math.random()(canvas.height-enemyHeight),
width:enemyWidth,height:enemyHeight,img:choice.img,speed:scrollSpeed+2+level0.6};
...
ctx.save();
ctx.fillStyle='white';
const fontSize = Math.max(canvas.height0.025,16);
ctx.font=fontSize+'px Arial';
const padX=canvas.width0.02, padY=canvas.height0.02;
let x=padX, y=padY+fontSize0.2;
ctx.textAlign='left'; ctx.textBaseline='top';
ctx.fillText('Lives:', x, y);
x += ctx.measureText('Lives:').width+canvas.width0.01;
const heartSize=Math.max(canvas.height0.03,18), gap=Math.max(canvas.width0.01,8);
for(let i=0;i<player.lives;i++) ctx.drawImage(heartImg, x+i(heartSize+gap), padY, heartSize, heartSize);
x+=player.lives*(heartSize+gap)+canvas.width0.02;
ctx.fillText('Level: '+level,x,y);
x+=ctx.measureText('Level: '+level).width+canvas.width0.02;
...
const titleSize=Math.max(canvas.height0.06,24);
ctx.font=titleSize+'px Arial';
ctx.fillText('using the arrow keys/joystick',canvas.width/2,canvas.height0.09);
ctx.fillText('Dodge the farmyard animals to rescue food for Moon Base 1',canvas.width/2,canvas.height0.09+titleSize1.2);
ctx.fillText('but watch out for that Crazy Chicken!!',canvas.width/2,canvas.height0.09+titleSize2.4);
const subSize=Math.max(canvas.height0.045,18);
ctx.font=subSize+'px Arial'; ctx.fillText('Choose Player',canvas.width/2,canvas.height0.32);
const imgSize=Math.max(canvas.height0.16,100), labelSize=Math.max(canvas.height0.04,16);
const spacing=Math.min(canvas.width0.28,imgSize1.6), startX=canvas.width/2-spacing, yImg=canvas.height0.42;
characterBounds.length=0;
characters.forEach((ch,i)=>{
const xCenter=startX+ispacing, xImg=xCenter-imgSize/2;
ctx.drawImage(ch.img,xImg,yImg,imgSize,imgSize);
ctx.font=labelSize+'px Arial';
const labelY=yImg+imgSize+labelSize1.2;
ctx.fillText(ch.label,xCenter,labelY);
const pad=imgSize0.1, textHeight=labelSize1.2;
const blockLeft=xImg-pad, blockWidth=imgSize+pad2;
characterBounds.push({x:blockLeft,y:yImg-pad,w:blockWidth,h:imgSize+textHeight+pad2,index:i});
if(selectedPlayer===i){
ctx.strokeStyle='yellow';
ctx.lineWidth=Math.max(canvas.height0.005,3);
...
if (drawHighScoreConsole.cursorVisible) nameText += "";
...
// finalize and save
drawHighScoreConsole.editingIndex = null;
resetHighScoreEditing();
